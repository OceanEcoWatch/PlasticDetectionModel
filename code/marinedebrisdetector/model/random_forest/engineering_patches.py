...
def indices(image):
    output_path = os.path.join(
        up(up(up(image))), "indices", "_".join(os.path.basename(image).split("_")[:-1])
    )
    output_image = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_si.tif"
    )
    ...
    dst_conf = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_conf.tif"
    )
    copyfile(src_conf, dst_conf)

    src_cl = os.path.abspath(image.split(".tif")[0] + "_cl.tif")
    dst_cl = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_cl.tif"
    )
...
def texture(image, window_size=13, max_value=16):
    output_path = os.path.join(
        up(up(up(image))), "texture", "_".join(os.path.basename(image).split("_")[:-1])
    )
    output_image = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_glcm.tif"
    )
    os.makedirs(output_path, exist_ok=True)
    ...
    with rasterio.open(output_image, "w", **meta) as dst:
        with rasterio.open(image, mode="r") as src:
            ...
            for col in range(
                (window_size - 1) // 2, gray.shape[0] + (window_size - 1) // 2
            ):
                for row in range(
                    (window_size - 1) // 2, gray.shape[0] + (window_size - 1) // 2
                ):
                    temp_gray_window = temp_gray[
                        row - (window_size - 1) // 2 : row + (window_size - 1) // 2 + 1,
                    ...
                    # So calculation on all directions and with 1 pixel offset-distance
                    matrix_coocurrence = skimage.feature.graycomatrix(
                        inds,
                        [1],
                    ...
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 0
                    ] = con
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 1
                    ] = dis
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 2
                    ] = homo
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 3
                    ] = ener
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 4
                    ] = cor
                    features_results[
                        row - (window_size - 1) // 2, col - (window_size - 1) // 2, 5
                    ] = asm
...
def spatial(image, sigma_min=1, sigma_max=16):
    output_path = os.path.join(
        up(up(up(image))), "spatial", "_".join(os.path.basename(image).split("_")[:-1])
    )
    output_image = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_spatial.tif"
    )
    os.makedirs(output_path, exist_ok=True)
...
def lbp(image, radius=3, n_points=24):
    output_path = os.path.join(
        up(up(up(image))), "lbp", "_".join(os.path.basename(image).split("_")[:-1])
    )
    output_image = os.path.join(
        output_path, os.path.basename(image).split(".")[0] + "_lbp.tif"
    )
    os.makedirs(output_path, exist_ok=True)
    ...
    with rasterio.open(output_image, "w", **meta) as dst:
        with rasterio.open(image, mode="r") as src:
            ...
            features_results_default = local_binary_pattern(
                gray, n_points, radius, "default"
            ).astype(dtype)
            features_results_uniform = local_binary_pattern(
                gray, n_points, radius, "uniform"
            ).astype(dtype)
...
def main(options):
    ...
    if options["type"] == "indices":
        ...
        Parallel(n_jobs=options["n_jobs"])(
            delayed(texture)(image, options["window_size"], options["max_value"])
            for image in tqdm(patches)
        )

    elif options["type"] == "spatial":
        Parallel(n_jobs=options["n_jobs"])(
            delayed(spatial)(image, 1, 16) for image in tqdm(patches)
        )

    elif options["type"] == "lbp":
        Parallel(n_jobs=options["n_jobs"])(
            delayed(lbp)(image, options["radius"], options["n_points"])
            for image in tqdm(patches)
        )
...
if __name__ == "__main__":
    ...
    # Options
    parser.add_argument(
        "--path", default=os.path.join(root_path, "data"), help="Path to dataset"
    )
    parser.add_argument(
        "--type",
        default="indices",
        type=str,
        help=" Select between indices or texture or spatial or lbp",
    )
    ...
    parser.add_argument(
        ...
        default=13,
        type=int,
        help="Size of the sliding window for the GLCM (use an odd number)",
    )
    parser.add_argument(
        "--max_value",
        default=16,
        type=int,
        help=(
            " Number of bins-levels for image quantization for the GLCM (use a power of"
            " two)"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="Radius of circle (spatial resolution of the operator)",
    )
    parser.add_argument(
        "--n_points",
        default=24,
