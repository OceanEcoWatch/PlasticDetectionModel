...
def parse_args():
    ...
    parser.add_argument(
        "--comparison", type=str, default="ours", choices=["ours", "mifdal"]
    )
    parser.add_argument(
        "--ckpt-folder", type=str, default="/data/marinedebris/checkpoints/unet++1"
    )
...
def parse_checkpoint_files_return_best(
    ckpt_files, metric="auroc", top_k=1, lower_better=False
):
    """
...
class EnsembleModel(nn.Module):
    def __init__(self, checkpoint_files):
        ...
        self.models = nn.ModuleList(
            [
                SegmentationModel.load_from_checkpoint(checkpoint_path=ckpt).model
...
def main(args):
    ...
    if args.comparison == "ours":
        ckpt_files = [
            f
            for f in os.listdir(args.ckpt_folder)
        ...
        model = SegmentationModel.load_from_checkpoint(
            checkpoint_path=os.path.join(args.ckpt_folder, ckpt_file)
        )

        if ensemble:
            checkpoint_files = [
                os.path.join(args.ckpt_folder, ckpt_file) for ckpt_file in ckpt_files
            ]
    ...
    logger = pl.loggers.csv_logs.CSVLogger(
        args.ckpt_folder, name="test_log", version=args.topk - 1
    )
    ...
    if args.calculate_qualitative:
        ...
        for name, path in marinedebris_datamodule.get_prediction_scene_paths():
            predpath = os.path.join(
                args.ckpt_folder, "test_scenes", name + "_prediction.tif"
            )
