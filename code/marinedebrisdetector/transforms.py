    ...
    if mode in ["train"]:
        ...
        def train_transform(image, mask=None):
            ...
            data_augmentation = get_data_augmentation(
                intensity=intensity, cropsize=cropsize
            )
...
def get_data_augmentation(intensity, cropsize):
    ...
    def data_augmentation(image, mask=None):
        ...
        if intensity >= 1:
            ...
            min_scale_factor = (cropsize + 5) / image.shape[
                1
            ]  # clamp scale factor so that random crop to certain cropsize is still possible
