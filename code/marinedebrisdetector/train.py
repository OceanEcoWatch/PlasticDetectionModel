...
def main(args):
    ...
    logger = WandbLogger(
        project=args.project, name=run_name, log_model=True, save_code=True
    )
    # logger.watch(model)
    ...
    plp_callback_2021 = PLPCallback(
        logger, marinedebris_datamodule.get_plp_dataset(2021)
    )
    plp_callback_2022 = PLPCallback(
        logger, marinedebris_datamodule.get_plp_dataset(2022)
    )

    qual_image_callback = RefinedRegionsQualitativeCallback(
        logger, marinedebris_datamodule.get_qualitative_validation_dataset()
    )
