...
URLs = {
    ...
    "S2SHIPS": (
        "https://marinedebrisdetector.s3.eu-central-1.amazonaws.com/data/S2SHIPS.zip"
    ),
    "MARIDA": (
        "https://marinedebrisdetector.s3.eu-central-1.amazonaws.com/data/MARIDA.zip"
    ),
}
...
class MarineDebrisDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_root: str = "/data/marinedebris",
        batch_size: int = 32,
        ...
        self.durban_scene = os.path.join(
            self.data_root, "durban", "durban_20190424.tif"
        )
        self.durban_l1c_scene = os.path.join(
            self.data_root, "durban", "durban_20190424_l1c.tif"
        )
        self.accra_scene = os.path.join(
            self.data_root, "marinedebris_refined", "accra_20181031.tif"
        )
    ...
    def prepare_data(self):
        if not self.download:
            if not os.path.exists(self.data_root):
                raise ValueError(
                    f"{self.data_root} does not exist. please check the path again or"
                    " specify download=True to download the data"
                )
        else:
    ...
    def setup(self, stage=None):
        train_transform = get_transform(
            "train", intensity=self.augmentation_intensity, cropsize=self.image_size
        )
        image_load_size = int(
            self.image_size * 1.2
        )  # load images slightly larger to be cropped later to image_size
        flobs_dataset = FloatingSeaObjectDataset(
            self.flobs_path,
            fold="train",
            transform=train_transform,
            refine_labels=not self.no_label_refinement,
            output_size=image_load_size,
        )
        shipsdataset = S2Ships(
            self.s2ships_path, imagesize=image_load_size, transform=train_transform
        )
        maridadataset = MaridaDataset(
            self.maridapath,
            imagesize=image_load_size,
        ...
        refinedflobs_val = RefinedFlobsDataset(
            root=self.refined_flobs_path, fold="val", shuffle=True
        )
        marida_val = MaridaDataset(
            self.maridapath,
            fold="val",
        ...
        self.valid_dataset = ConcatDataset([refinedflobs_val, marida_val])
        maridatestdataset = MaridaDataset(
            self.maridapath,
            fold="test",
            imagesize=self.image_size,
            data_transform=test_transform,
        ...
        flobstestdataset = RefinedFlobsDataset(
            root=self.refined_flobs_path, fold="test", shuffle=True
        )
        ...
        print("val")
        non_debris = sum(
            [(ds.points["type"] == 0).sum() for ds in refinedflobs_val.datasets]
        )
        ...
        print("test")
        non_debris = sum(
            [(ds.points["type"] == 0).sum() for ds in flobstestdataset.datasets]
        )
    ...
    def get_qualitative_validation_dataset(self, output_size=256):
        return RefinedFlobsQualitativeDataset(
            root=self.refined_flobs_path, fold="val", output_size=output_size
        )

    def get_qualitative_test_dataset(self, output_size=256):
        return RefinedFlobsQualitativeDataset(
            root=self.refined_flobs_path, fold="test", output_size=output_size
        )
