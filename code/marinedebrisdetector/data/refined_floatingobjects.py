...
test_regions = ["accra_20181031", "durban_20190424"]
...
class RefinedFlobsRegionDataset(Dataset):
    def __init__(self, root, region, imagesize=1280, shuffle=False, transform=None):
        ...
        out_of_bounds = pd.concat(
            [
                image_bounds.minx < left,
                image_bounds.miny < bottom,
                image_bounds.maxx > right,
                image_bounds.maxy > top,
            ],
            axis=1,
        ).any(axis=1)
...
class RefinedFlobsDataset(ConcatDataset):
    def __init__(self, root, fold="val", **kwargs):
        ...
        super().__init__(
            [
                RefinedFlobsRegionDataset(root, region, **kwargs)
...
class RefinedFlobsQualitativeRegionDataset(Dataset):
    def __init__(self, root, region, output_size=None):
        ...
        self.region = region
        self.output_size = (
            output_size  # overwrite geometry and fix a squared output size
        )
        ...
        self.segments = (
            gpd.read_file(segments_file) if os.path.exists(segments_file) else None
        )
        self.segments_tiffile = (
            os.path.join(root, region + "_qualitative_poly.tif")
...
class RefinedFlobsQualitativeDataset(ConcatDataset):
    def __init__(self, root, fold="val", **kwargs):
        ...
        super().__init__(
            [
                RefinedFlobsQualitativeRegionDataset(root, region, **kwargs)
                for region in self.regions
            ]
        )
...
def main():
    ds = RefinedFlobsQualitativeDataset(
        root="/data/marinedebris/marinedebris_refined", fold="val"
    )
    for i, id in ds:
        print(i.shape, id)

    ds = RefinedFlobsQualitativeDataset(
        root="/data/marinedebris/marinedebris_refined", fold="test"
    )
    for i, id in ds:
        print(i.shape, id)

    return
    ds = RefinedFlobsRegionDataset(
        root="/data/marinedebris/marinedebris_refined", region="accra_20181031"
    )
