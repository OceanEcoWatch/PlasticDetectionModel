...
from .utils import get_window, line_is_closed, read_tif_image
...
refinement_args_list = [
    dict(buffersize_water=w, rw_beta=beta, object_seed_probability=seed)
...
def main(root="/data/marinedebris/floatingobjects"):
    ...
    shapefiles = [
        os.path.join(shapefile_path, shp)
        for shp in os.listdir(shapefile_path)
        if shp.endswith("shp")
    ]
    ...
    process_map(
        refine_worker,
        zip(shapefiles, imagetiffiles, masktifffiles),
        max_workers=16,
...
def refine_masks(
    ...
    if (otsu_segments * mask_lines).sum() > 0:
        seeds_lines = (
            otsu_segments
            * mask_lines
            * (np.random.rand(*out_shape) > object_seed_probability)
        )
    else:  # otherwise ignore otsu segments
        seeds_lines = mask_lines * (
            np.random.rand(*out_shape) > object_seed_probability
        )

    if (
        seeds_lines.sum() > 0
    ):  # if line seeds present, otherwise fall back to returning the original mask
        markers = seeds_lines * 1 + seeds_water * 2
        labels = (
            random_walker(
                fdi_image, markers, beta=rw_beta, mode="bf", return_full_prob=False
            )
            == 1
        )
    else:
...
def refine_masks_iterative(imagetiffile, masktifffile, geometries, patch_size=256):
    ...
    with rasterio.open(masktifffile, "w+", **maskprofile) as dst:
        for geometry in geometries:
            ...
            mask_rasterized = features.rasterize(
                geometries,
                all_touched=True,
            ...
            if (
                mask_rasterized.shape[0] != patch_size
                or mask_rasterized.shape[1] != patch_size
            ):
            ...
            mask_refined = np.vstack(
                [
                    refine_masks(image, mask_rasterized, **refinement_args)[None]
                    for refinement_args in refinement_args_list
                ]
            )
            mask_previous = dst.read(window=window)[
                1:
            ]  # take previously written refined masks (excluding first band containing the original mask)
            ...
            for i, desc in enumerate(refinement_args_list):
                dst.set_band_description(
                    i + 2,
                    f"buff{desc['buffersize_water']}_beta{desc['rw_beta']}_pseed{desc['object_seed_probability']}",
                )
...
def initialize_mask(imagetiffile, masktifffile):
    ...
    with rasterio.open(imagetiffile, "r") as src:
        profile = src.meta
        profile.update(
            count=len(refinement_args_list) + 1, dtype="uint8", compression="lzw"
        )

    with rasterio.open(masktifffile, "w+", **profile) as dst:
        dst.write(
            np.zeros(
                (len(refinement_args_list) + 1, profile["height"], profile["width"])
            )
        )
