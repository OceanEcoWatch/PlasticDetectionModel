...
from .utils import pad, read_tif_image
...
REGIONS = [
    ...
    "S2_7-10-18_52SDD",
    "S2_7-3-20_18QYG",
    "S2_9-10-17_16PECS2_12-1-17_16PEC",
    "S2_12-1-17_16PCC",
    "S2_19-9-18_16PCC",
    "S2_19-9-18_16PDC",
    "S2_4-3-18_50LLR",
...
class MaridaRegionDataset(Dataset):
    def __init__(
        self, path, region, imagesize=128, data_transform=None, classification=False
    ):
        ...
        if not os.path.exists(self.maskpath):
            mask = rasterize(
                zip(self.gdf.geometry, self.gdf.id),
                all_touched=True,
                transform=transform,
    ...
    def __getitem__(self, item):
        ...
        minx, miny, maxx, maxy = row.geometry.centroid.buffer(
            self.imagesize // 2
        ).bounds
...
class MaridaDataset(ConcatDataset):
    def __init__(self, path, fold="train", **kwargs):
        ...
        self.regions = list(
            set(["S2_" + "_".join(i.replace("\n", "").split("_")[:-1]) for i in lines])
        )
