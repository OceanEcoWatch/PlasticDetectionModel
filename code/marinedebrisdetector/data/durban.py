...
class DurbanDataset(torch.utils.data.Dataset):
    def __init__(self, root, output_size=64, transform=None, objects="debris"):
        if objects == "debris":
            ...
            raise ValueError(
                f"specified objects {objects} not valid. either `debris` or ships"
            )
...
def write_annotations_file(
    ...
    profile.update(count=len(annotated_objects), dtype="uint8")
